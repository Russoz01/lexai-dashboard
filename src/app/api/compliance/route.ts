...
export async function POST(req: NextRequest) {
  ...
  try {
    ...
    // Plan-based access (Wave R1 audit): Compliance é Firma+
    const planBlock = await assertPlanAccess(supabase, user.id, 'compliance')
    ...
    if (isDemoFallbackEnabled() && isRetryableError(err)) {
      return NextResponse.json(getDemoFallback('compliance', { reason: errMsg }))
    }
