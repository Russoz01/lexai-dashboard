...
export async function POST(req: NextRequest) {
  ...
  try {
    ...
    const planBlock = await assertPlanAccess(supabase, user.id, 'simulado')
    ...
    const resultado = responseText
      ? parseAgentJSON<Record<string, unknown>>(responseText, { parecer: null, erro_parse: true, raw: responseText })
      : { parecer: null, erro_parse: true }
    ...
    if (isDemoFallbackEnabled() && isRetryableError(err)) {
      return NextResponse.json(getDemoFallback('simulado', { reason: errMsg }))
    }
